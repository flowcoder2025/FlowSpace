...
> **상태**: ✅ **완료** - 27/38 수정 완료 (71%), 11개 인프라 의존으로 보류
> **다음 태스크**: 인프라 마이그레이션 후 보류 이슈 해결 (OCI.md 참조)
...
| Phase 3 | Medium | 17 | 10 | 7 | 59% ✅ |
| Phase 4 | Low | 7 | 3 | 4 | 43% ✅ |
| **합계** | - | **38** | **27** | **11** | **71%** |

**Phase 3 완료 항목**: 3.1, 3.3, 3.4, 3.5, 3.6, 3.10, 3.11, 3.14, 3.16, 3.17
**Phase 3 보류 항목**: 3.2 (최적화), 3.7-3.9 (복잡/낮은 우선순위), 3.12-3.13 (문제 없음), 3.15 (낮은 위험도)
**Phase 4 완료 항목**: 4.3, 4.5, 4.6
**Phase 4 보류 항목**: 4.1 (DB 인덱스), 4.2 (이론적 위험), 4.4 (인프라), 4.7 (cron job)
...
- **파일**: `src/app/api/admin/stats/route.ts:77-219`
- **문제**: 같은 조건의 ENTER/EXIT 4번 반복 조회
- **해결**: Promise.all로 병렬 실행 중 (성능상 문제 없음, 리팩토링은 향후)
- **상태**: ⏸️ 보류 (최적화, 기능 이슈 아님)
...
### 3.7 [채팅] 프로필 업데이트/이동 경합
- **파일**: `server/socket-server.ts:994-1034`
- **문제**: 동시 이벤트 순서 미보장
- **해결**: (향후) 이벤트 시퀀스 번호 추가 필요
- **상태**: ⏸️ 보류 (복잡한 구현, 현재 큰 문제 아님)

### 3.8 [채팅] 파티 채팅 권한 검증 없음
- **파일**: `server/socket-server.ts:892-952`
- **문제**: 누구나 파티 참가 가능
- **해결**: (향후) 초대 기반 파티 참가로 변경 필요
- **상태**: ⏸️ 보류 (파티 기능 확장 시 구현)

### 3.9 [채팅] 음소거 상태 메모리만 저장
- **파일**: `server/socket-server.ts:1101-1167`
- **문제**: 서버 재시작 시 음소거 초기화
- **해결**: (향후) SpaceMember.restriction 필드 사용 필요
- **상태**: ⏸️ 보류 (관리자 기능 확장 시 구현)
...
### 3.15 [인증] 사용자 열거 가능
- **파일**: `src/app/api/spaces/[id]/staff/route.ts`
- **문제**: 존재하지 않는 userId에 대한 404 응답
- **해결**: (향후) 일관된 에러 응답으로 변경 필요
- **상태**: ⏸️ 보류 (낮은 위험도)
...
## Phase 4: Low (분기 내 수정) - ✅ 완료 (3/7)

### 4.1 [통계] authReturnRateData groupBy 정렬 비용
- **파일**: `src/app/api/admin/stats/route.ts:210-218`
- **문제**: 대규모 데이터셋에서 성능 저하
- **해결**: 인덱스 최적화 또는 캐싱 (DB 스키마 변경 필요)
- **상태**: ⏸️ 보류 (인프라/스키마 변경)

### 4.2 [통계] 스키마 마이그레이션 시 세션 ID 재사용 위험
- **파일**: `src/app/api/admin/stats/route.ts:264-272`
- **문제**: cuid() 재사용 가능성 (이론적)
- **해결**: cuid()는 이미 충분히 고유함 (약 10^36 경우의 수)
- **상태**: ⏸️ 보류 (이론적 위험, 실제 문제 없음)
...
- **해결**: cron job 또는 정리 스케줄러 필요
- **상태**: ⏸️ 보류 (인프라/스케줄러 추가 필요)
...
**보류된 11개 이슈 해결 조건**:
1. Oracle Cloud 인프라 배포 완료 (OCI.md 참조)
2. cron job / 스케줄러 설정
3. DB 스키마 마이그레이션
