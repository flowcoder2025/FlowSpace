...
└── /space             # 📌 공간 기능 (핵심 모듈)
    ...
    │   ├── /chat               # 📌 플로팅 채팅 (2025-12 개편)
    │   │   ├── FloatingChatOverlay.tsx  # 드래그 가능 오버레이
    ...
    ├── /hooks         # 공간 관련 훅
    │   ├── useChatMode.ts      # 채팅 모드 (ACTIVE/INACTIVE)
    │   ├── useChatDrag.ts      # 드래그 위치, localStorage
...
| `FloatingChatOverlay` | 📌 드래그 가능 플로팅 채팅창 (게임 캔버스 위) |
| `ChatTabs` | 전체/귓속말/파티 탭 전환 |
| `ParticipantPanel` | 우측 참가자 비디오 그리드 |
| `VideoTile` | 개별 비디오 타일 (CSS hue-rotate 아바타 색상) |
...
| 날짜 | 변경 |
|-----|------|
| 2025-12-11 | 채팅 시스템 개편 반영: FloatingChatOverlay, ChatTabs, 채팅 훅 문서화 |
| 2025-12-11 | 디렉토리 구조 업데이트: /chat 폴더 추가, /sidebar 레거시 표시 |
| 2025-12-05 | 초기 생성 |
